...
gulp.task('build', ['highlight'], function () {
  ...
  return browserify('./src/ponymark.js')
    .bundle({ debug: true, standalone: 'ponymark' })
    .pipe(source('ponymark.js'))
    .pipe(streamify(header(extended, { pkg : pkg } )))
    .pipe(gulp.dest('./dist'))
    .pipe(streamify(rename('ponymark.min.js')))
...
gulp.task('highlight', ['styles'], function () {
  var pkg = require('./package.json');
